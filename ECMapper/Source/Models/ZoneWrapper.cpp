...
void ZoneWrapper::addListener(DeviceType deviceType, juce::ValueTree::Listener *listener, juce::ValueTree &rootState) {
    auto vTree = getZoneTree(deviceType, Zone::Zone1, rootState);
    vTree.addListener(listener);
    vTree = getZoneTree(deviceType, Zone::Zone2, rootState);
    vTree.addListener(listener);
    vTree = getZoneTree(deviceType, Zone::Zone3, rootState);
    vTree.addListener(listener);
}

juce::ValueTree ZoneWrapper::getZoneTree(DeviceType deviceType, Zone zone, juce::ValueTree &rootState) {
...
MidiChannelType ZoneWrapper::getMidiChannelType(DeviceType deviceType, Zone zone, juce::ValueTree &rootState) {
...
void ZoneWrapper::setMidiChannelType(DeviceType deviceType, Zone zone, MidiChannelType midiChannelType, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    zoneTree.setProperty(id_midiChannelType, (int)midiChannelType, nullptr);
}

int ZoneWrapper::getTranspose(DeviceType deviceType, Zone zone, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return default_transpose;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    return zoneTree.getProperty(id_transpose, default_transpose);
}

void ZoneWrapper::setTranspose(DeviceType deviceType, Zone zone, int value, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    zoneTree.setProperty(id_transpose, value, nullptr);
}

void ZoneWrapper::setKeyPitchbend(DeviceType deviceType, Zone zone, int value, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    zoneTree.setProperty(id_keyPitchbend, value, nullptr);
}

int ZoneWrapper::getKeyPitchbend(DeviceType deviceType, Zone zone, juce::ValueTree &rootState) {
...
void ZoneWrapper::setChannelMaxPitchbend(DeviceType deviceType, Zone zone, int value, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    zoneTree.setProperty(id_channelMaxPitchbend, value, nullptr);
}

int ZoneWrapper::getChannelMaxPitchbend(DeviceType deviceType, Zone zone, juce::ValueTree &rootState) {
...
void ZoneWrapper::setEnabled(DeviceType deviceType, Zone zone, bool enabled, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    zoneTree.setProperty(id_enabled, enabled, nullptr);
}

bool ZoneWrapper::getEnabled(DeviceType deviceType, Zone zone, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return default_enabled;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
    return zoneTree.getProperty(id_enabled, default_enabled);
}

ZoneWrapper::MidiValue ZoneWrapper::getMidiValue(DeviceType deviceType, Zone zone, juce::Identifier childId, ZoneWrapper::MidiValue defaultValue, juce::ValueTree &rootState) {
...
void ZoneWrapper::setMidiValue(DeviceType deviceType, Zone zone, juce::Identifier childId, MidiValue midiValue, juce::ValueTree &rootState) {
    if (deviceType == DeviceType::None) return;
    auto zoneTree = getZoneTree(deviceType, zone, rootState);
